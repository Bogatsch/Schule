...
export function setupVideoAnnotation({ statusElement = null } = {}) {
  ...
  function showStatus(message, target = statusElement) {
    if (target) {
  ...
  function open(video, label, sourceButton, openStatusElement = statusElement) {
    if (!video || video.readyState < 2 || !video.videoWidth || !video.videoHeight) {
      showStatus(
        'Der aktuelle Videoframe ist noch nicht verfügbar. Starte das Video kurz und versuche es erneut.',
        openStatusElement
      );
      return false;
    }
    ...
    try {
      ...
      clearCanvases();
      showStatus('Der aktuelle Videoframe konnte nicht übernommen werden.', openStatusElement);
      return false;
    }
